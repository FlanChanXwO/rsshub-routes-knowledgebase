# 中华人民共和国国家发展和改革委员会 Route Index

## Namespace
- Namespace: `gov/ndrc`
- Display Name: `中华人民共和国国家发展和改革委员会`
- URL: `ndrc.gov.cn`
- Language: `_None_`
- Aliases: `gov/ndrc, ndrc, ndrc.gov.cn, 中华人民共和国国家发展和改革委员会`
- Route Count: `3`

## Routes

### 发展改革工作
- Route ID: `gov/ndrc:/gov/ndrc/fggz/:category{.+}?`
- Route Path: `/gov/ndrc/fggz/:category{.+}?`
- File: `docs/routes/gov-ndrc/gov-ndrc-fggz-category.md`
- File Name: `gov-ndrc-fggz-category.md`
- Categories: `government`
- Maintainers: `nczitzk`

### 新闻动态
- Route ID: `gov/ndrc:/gov/ndrc/xwdt/:category{.+}?`
- Route Path: `/gov/ndrc/xwdt/:category{.+}?`
- File: `docs/routes/gov-ndrc/gov-ndrc-xwdt-category.md`
- File Name: `gov-ndrc-xwdt-category.md`
- Categories: `government`
- Maintainers: `nczitzk`

### 政府信息公开
- Route ID: `gov/ndrc:/gov/ndrc/zfxxgk`
- Route Path: `/gov/ndrc/zfxxgk`
- File: `docs/routes/gov-ndrc/gov-ndrc-zfxxgk.md`
- File Name: `gov-ndrc-zfxxgk.md`
- Categories: `government, popular`
- Maintainers: `howfool, nczitzk`
